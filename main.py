...
layout = [[sg.Text('Введите размеры лабиринта и  (по желанию)'),
          sg.Text('координаты (через запятую) старта и финиша')],
          [sg.Text('n:', size=(10, 1)), sg.InputText(),
          sg.Text('m:', size=(10, 1)), sg.InputText()],
          [sg.Text('start:', size=(10, 1)), sg.InputText(),
          sg.Text('finish:', size=(10, 1)), sg.InputText()],
          [sg.Text('Выберите алгоритм генерации:',size=(30, 1)),
          sg.InputCombo(('dfs', 'mst'), size=(20, 3))],
          [sg.Text('имя файла:', size=(15, 1)), sg.InputText()],
          [sg.Button('Генерировать'), sg.Button('Вывести'),
          sg.Button('Решить'), sg.Button('Сохранить')],
...
while True:
    ...
    if event == 'Генерировать':
        if values[0] == '' or values[1] == '' or values[4] == '':
            ...
            try:
                ...
                print('Введены некорректные размеры лабиринта.',
                    'Должно быть два натуральных числа')
                sg.popup('Введены некорректные размеры лабиринта.',
                    'Должно быть два натуральных числа')
                continue
            if values[2] == '':
                start = (0, 0)
                ...
                try:
                    ...
                    print('Введены некоректные координаты старта.',
                        'Они должны быть в промежутке [0, n), [0, m)')
                    sg.popup('Введены некоректные координаты старта.',
                        'Они должны быть в промежутке [0, n), [0, m)')
                    continue
            if values[3] == '':
                finish = 0
                ...
                try:
                    ...
                    print('Введены некоректные координаты финиша.',
                        'Они должны быть в промежутке [0, n), [0, m)')
                    sg.popup('Введены некоректные координаты финиша.',
                        'Они должны быть в промежутке [0, n), [0, m)')
                    continue
            ...
            if values[4] == 'dfs':
                ...
                print('Введено некорректное название алгоритма.',
                    'Пожалуйста, выберите из предложенных')
                sg.popup('Введено некорректное название алгоритма.',
                    'Пожалуйста, выберите из предложенных')
                continue
            drawing.pr_matze(a)
    ...
    if event == 'Вывести':
        try:
            ...
            print('Что-то пошло не так.',
                'Возможно, вы еще не создали или не загрузили лабиринт')
    ...
    if event == 'Сохранить':
        ...
        try:
            ...
            print('Возможно лабиринт не сгенерирован',
                'или введено некорректное имя файла')
            sg.popup('Возможно лабиринт не сгенерирован',
                'или введено некорректное имя файла')
